...
def showDatas():
    ...
    ### 커넥션 객체생성 > 커서 > 쿼리실행 > 커서로 데이터 패치 > 커서종료 > 커넥션 종료
    ### 1) DB연결. 
    ...
    ### 3) 쿼리문 작성
    query_select = 'select std_id, std_name, std_mobile, std_regyear from students'
    cursor.execute(query_select) # query = query 쿼리실행
...
# 6. getData(event) - 트리뷰 한줄 더블클릭 한 값 엔트리에 표시
def getData(event):
    '''
    ...
    ## 엔트리 위젯 기존 내용 삭제
    ent1.config(state='normal') # 수정 가능 상태
    ent1.delete(0, END) # 학생번호 기존 데이터 삭제
    ent1.config(state='readonly') # 수정 불가능 상태
    ...
    if sel_item:
        item_values = dataView.item(sel_item, 'values') # 선택항목 'values'(실 데이터) 가져오기 sel_item = ('I002')
    ...
    ent3.insert(0, item_values[2]) # 휴대폰
...
def addData():
    ...
    ## 엔트리 위젯 학생정보데이터 변수 할당
    std_name = ent2.get() # 학생명
    std_mobile = ent3.get() # 휴대폰
    std_regyear = ent4.get() # 입학년도

    if std_name == '' or std_mobile == '' or std_regyear == '':
        messagebox.showwarning('SELECT', '추가할 데이터 입력요망!')
        return 
    ...
    try:
        ...
        val = (std_name, std_mobile, std_regyear) # 튜플
        cursor.execute(query = query, args=val) # val의 데이터를 매개변수로 해서 query 실행
    ...
    showDatas() # DB 테이블의 모든 데이터를 조회해서 트리뷰에 표시
...
def modData():
    ...
    try:
        ...
        val = (std_name, std_mobile, std_regyear, std_id) # 튜플
        cursor.execute(query = query, args=val) # val의 데이터를 매개변수로 해서 query 실행
        ...
        messagebox.showinfo('UPDATE', '정보수정 성공!')
        ...
        print(e)
        conn.rollback() # 트랜잭션 롤백
        messagebox.showerror('UPDATE', '정보수정 실패!!')
    finally:
        cursor.close()  # 커서
        conn.close()    # 커넥션 종료
...
def delData():
    ...
    if std_id == '':
        messagebox.showwarning('DELETE', '삭제할 데이터 선택요망!')
        return 
...
root.resizable(False, False)    # 윈도우 사이즈(가로, 세로) 변경 불가
root.iconbitmap('./image/students.ico') # 아이콘 등록

myFont = Font(family='NanumGothic', size = 10)  # 이후에 화면 위젯에 지정할 동일 폰트 생성

# 4. UI 구성
## 4-1. label
tk.Label(root, text = '학생번호', font=myFont).place(x=10, y=10) # x축10, y축10 위치에 학생번호 레이블 생성
...
## 4-2. 엔트리(텍스트박스) entry는 한번에 설정으로 오동작시 따로 설정 
ent1 = tk.Entry(root, font=myFont)
ent1 = tk.Entry(state='readonly', fg='blue', disabledbackground='blue') # ent1.config(state='readonly') # 값을 입력 받지 못하게 설정
...
ent3.place(x=140, y=70) # 휴대폰 Entry
ent4 = tk.Entry(root, font=myFont)
ent4.place(x=140, y=100) # 입학년도 Entry

## 4-3. 버튼
## 7-1. 추가 버튼에 addData() 함수를 연결
tk.Button(root, text='추가', font=myFont, height=2, width=10, command=addData).place(x=10, y=130) # 추가버튼 30
## 8-1. 수정 버튼에 modData() 함수 연결
tk.Button(root, text='수정', font=myFont, height=2, width=10, command=modData).place(x=110, y=130) # 수정버튼 140
## 9-1. 삭제 버튼에 modData() 함수 연결
tk.Button(root, text='삭제', font=myFont, height=2, width=10, command=delData).place(x=210, y=130) # 삭제버튼 250
# tk.Button(root, text='정렬', font=myFont, height=1, width=10).place(x=310, y=130) # 정렬버튼 250
## 4-4. 트리뷰 (행과 열로 만들어진 데이터 표현 시 좋은 위젯 중 하나)
cols = ('학생번호', '학생명', '핸드폰', '입학년도')
dataView = ttk.Treeview(root, columns=cols, show='headings', height=14)

## 4-5. 트리뷰 설정
for col in cols: # 학생번호~입학년도
...
# 6. 트리뷰 항목 더블클릭시 이벤트 발생(getData()함수 호출)
dataView.bind('<Double-Button-1>', func=getData)

# 10. 최초 실행 시 ent2(학생명)에 포커스
ent2.focus_set()
